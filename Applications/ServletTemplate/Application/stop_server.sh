...
# stop_server.sh
# 
# Use this script to stop a ServletTemplate.
#
# Usage: ./stop_server.sh    Stops the ServletTemplate.
...
if [ "$check_run" = "0" ]; then
  ...
  echo "${red}[ERROR]${reset} Could not terminate ${yellow}ServletTemplate${reset}."
  echo "        ${yellow}ServletTemplate${reset} is not active."
else
  # ServletTemplate is already running.
  pid=$(ps -ef | grep -i "ServletTemplate" | grep -v "grep" | grep -v "bash" | awk '{ print $2 }')
  kill -9 $pid
  echo "${yellow}ServletTemplate${reset} terminated."
