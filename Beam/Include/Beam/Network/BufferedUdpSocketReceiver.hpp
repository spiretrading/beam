#ifndef BEAM_BUFFERED_UDP_SOCKET_RECEIVER_HPP
#define BEAM_BUFFERED_UDP_SOCKET_RECEIVER_HPP
#include <cstdint>
#include <deque>
...
namespace Beam {
  ...
  class BufferedUdpSocketReceiver {
    public:

      /**
       * Constructs a BufferedUdpSocketReceiver.
       * @param options The options to apply to the receiver.
       * @param socket The socket to send the receive operations to.
       */
      BufferedUdpSocketReceiver(const UdpSocketOptions& options,
    ...
    private:
      ...
      BufferedUdpSocketReceiver(const BufferedUdpSocketReceiver&) = delete;
      BufferedUdpSocketReceiver& operator =(
        const BufferedUdpSocketReceiver&) = delete;
      template<IsBuffer R>
      std::size_t receive(
        Out<R> destination, std::size_t size, IpAddress* address);
  };

  inline BufferedUdpSocketReceiver::BufferedUdpSocketReceiver(
  ...
  inline bool BufferedUdpSocketReceiver::poll() const {
    auto lock = std::lock_guard(m_state->m_socket->m_mutex);
  ...
  template<IsBuffer R>
  std::size_t BufferedUdpSocketReceiver::receive(
      Out<DatagramPacket<R>> packet) {
  ...
  inline BufferedUdpSocketReceiver::State::State(
    const UdpSocketOptions& options,
    std::shared_ptr<Details::UdpSocketEntry> socket)
    : m_options(options),
  ...
  inline void BufferedUdpSocketReceiver::on_read(
      const std::shared_ptr<State>& state,
      const boost::system::error_code& error, std::size_t size) {
  ...
  inline void BufferedUdpSocketReceiver::on_deadline(
