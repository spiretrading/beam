#ifndef BEAM_LOCALPTR_HPP
#define BEAM_LOCALPTR_HPP
...
namespace Beam {
  ...
  /*! \class LocalPtr
      \brief Wraps a local value as if it were a pointer.
   */
  template<typename T>
  class LocalPtr {
    public:
  ...
  /*! \class OptionalLocalPtr
      \brief Evaluates to a LocalPtr if T is not dereferenceable.  Useful when
             a class must always work using 'pointer' semantics but is using
             a value behind the scenes.
   */
  template<typename T>
