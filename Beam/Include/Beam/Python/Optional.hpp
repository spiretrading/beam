#ifndef BEAM_PYTHONOPTIONAL_HPP
#define BEAM_PYTHONOPTIONAL_HPP
...
namespace Python {
  ...
  /*! \class python_optional
      \brief Exposes a boost::optional type to Python where the uninitialized
             case is represented as None.
      \author http://stackoverflow.com/questions/6274822/boost-python-no-to-python-converter-found-for-stdstring
   */
  template<typename T>
  struct python_optional : private boost::noncopyable {
    ...
    explicit python_optional() {
      ...
      if(!extract<boost::optional<T> >(object()).check()) {
        ...
        converter::registry::push_back(&convertible, &constructor,
          type_id<boost::optional<T>>(), &conversion::get_pytype);
      }
    }
  };
}
