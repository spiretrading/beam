#ifndef BEAM_SERVICELOCATORDATASTORE_HPP
#define BEAM_SERVICELOCATORDATASTORE_HPP
#include <string>
...
namespace ServiceLocator {
    ...
    public:
      ...
      */
      virtual DirectoryEntry MakeAccount(const std::string& name,
        ...
        const boost::posix_time::ptime& registrationTime) = 0;
  ...
  */
  inline DirectoryEntry LoadDirectoryEntry(ServiceLocatorDataStore& dataStore,
    ...
    std::vector<DirectoryEntry> children = dataStore.LoadChildren(root);
  ...
  */
  ...
  inline ServiceLocatorDataStore::~ServiceLocatorDataStore() {}

  inline DirectoryEntry ServiceLocatorDataStore::Validate(
      const DirectoryEntry& entry) {
    DirectoryEntry validatedEntry = LoadDirectoryEntry(entry.m_id);
