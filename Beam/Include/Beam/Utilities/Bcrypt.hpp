...
namespace Details {
  ...
  //! for all of the private implementation.
  #define _PASSWORD_LEN 128
  #define BLF_N 16
  #define BLF_MAXKEYLEN ((BLF_N - 2) * 4)
  struct blf_ctx {
    std::uint32_t S[4][256];
    std::uint32_t P[BLF_N + 2];
  };
  void Blowfish_encipher(blf_ctx*, std::uint32_t*, std::uint32_t*);
  void Blowfish_decipher(blf_ctx*, std::uint32_t*, std::uint32_t*);
  void Blowfish_initstate(blf_ctx*);
  void Blowfish_expand0state(blf_ctx*, const std::uint8_t*, std::uint16_t);
  void Blowfish_expandstate(blf_ctx*, const std::uint8_t*, std::uint16_t,
    const std::uint8_t*, std::uint16_t);
  void blf_key(blf_ctx*, const std::uint8_t*, std::uint16_t);
  void blf_enc(blf_ctx*, std::uint32_t*, std::uint16_t);
  void blf_dec(blf_ctx*, std::uint32_t*, std::uint16_t);
  void blf_ecb_encrypt(blf_ctx*, std::uint8_t*, std::uint32_t);
  void blf_ecb_decrypt(blf_ctx*, std::uint8_t*, std::uint32_t);
  void blf_cbc_encrypt(blf_ctx*, std::uint8_t*, std::uint8_t*, uint32_t);
  void blf_cbc_decrypt(blf_ctx*, std::uint8_t*, std::uint8_t*, uint32_t);
  std::uint32_t Blowfish_stream2word(const std::uint8_t*, std::uint16_t ,
    std::uint16_t*);
  ...
  inline void Blowfish_encipher(blf_ctx *c, std::uint32_t* xl,
      std::uint32_t* xr) {
    std::uint32_t Xl;
    std::uint32_t Xr;
  ...
  inline void Blowfish_decipher(blf_ctx *c, std::uint32_t* xl,
      std::uint32_t* xr) {
    uint32_t Xl;
    uint32_t Xr;
    uint32_t *s = c->S[0];
  ...
  inline void Blowfish_initstate(blf_ctx* c) {
  ...
  inline std::uint32_t Blowfish_stream2word(const std::uint8_t* data,
      std::uint16_t databytes, std::uint16_t* current) {
    std::uint8_t i;
    std::uint16_t j;
    std::uint32_t temp;
    temp = 0x00000000;
  ...
  inline void Blowfish_expand0state(blf_ctx* c, const std::uint8_t* key,
  ...
  inline void Blowfish_expandstate(blf_ctx* c, const std::uint8_t* data,
      std::uint16_t databytes, const std::uint8_t* key,
      std::uint16_t keybytes) {
    std::uint16_t i;
    std::uint16_t j;
    std::uint16_t k;
  ...
  inline void blf_key(blf_ctx* c, const std::uint8_t* k, std::uint16_t len) {
    Blowfish_initstate(c);
    Blowfish_expand0state(c, k, len);
  }

  inline void blf_enc(blf_ctx* c, std::uint32_t* data, std::uint16_t blocks) {
    std::uint32_t* d;
    std::uint16_t i;
    d = data;
    for(i = 0; i < blocks; i++) {
  ...
  inline void blf_dec(blf_ctx* c, std::uint32_t* data, std::uint16_t blocks) {
    std::uint32_t* d;
    std::uint16_t i;
    d = data;
    for(i = 0; i < blocks; i++) {
  ...
  inline void blf_ecb_encrypt(blf_ctx* c, std::uint8_t* data,
      std::uint32_t len) {
    std::uint32_t l, r;
    std::uint32_t i;
    for(i = 0; i < len; i += 8) {
  ...
  inline void blf_ecb_decrypt(blf_ctx* c, std::uint8_t* data,
      std::uint32_t len) {
    std::uint32_t l, r;
    std::uint32_t i;
    for(i = 0; i < len; i += 8) {
  ...
  inline void blf_cbc_encrypt(blf_ctx* c, std::uint8_t* iv, std::uint8_t* data,
      std::uint32_t len) {
    std::uint32_t l, r;
    std::uint32_t i, j;
    for(i = 0; i < len; i += 8) {
  ...
  inline void blf_cbc_decrypt(blf_ctx* c, std::uint8_t* iva, std::uint8_t* data,
      std::uint32_t len) {
    std::uint32_t l, r;
    std::uint8_t* iv;
    std::uint32_t i, j;
  ...
  static const std::uint8_t index_64[128] = {
  ...
  inline char* bcrypt(char* output, const char* key, const char* salt) {
    ...
    std::uint16_t j;
    std::uint8_t key_len, salt_len, logr, minor;
    std::uint8_t ciphertext[4 * BCRYPT_BLOCKS] = {'O', 'r', 'p', 'h', 'e', 'a',
      'n', 'B', 'e', 'h', 'o', 'l', 'd', 'e', 'r', 'S', 'c', 'r', 'y', 'D', 'o',
      'u', 'b', 't'};
    std::uint8_t csalt[BCRYPT_MAXSALT];
    ...
    logr = static_cast<std::uint8_t>(n);
    if((rounds = (uint32_t) 1 << logr) < BCRYPT_MINROUNDS) {
      return NULL;
    }
    salt += 3;
    if(std::strlen(salt) * 3 / 4 < BCRYPT_MAXSALT) {
      return NULL;
    }
    decode_base64(csalt, BCRYPT_MAXSALT, const_cast<std::uint8_t*>(
      reinterpret_cast<const std::uint8_t*>(salt)));
    salt_len = BCRYPT_MAXSALT;
    key_len = static_cast<uint8_t>(strlen(key) + (minor >= 'a' ? 1 : 0));
    Blowfish_initstate(&state);
    Blowfish_expandstate(&state, csalt, salt_len,
      reinterpret_cast<const uint8_t*>(key), key_len);
    for(k = 0; k < rounds; k++) {
      Blowfish_expand0state(&state, reinterpret_cast<const uint8_t*>(key),
        key_len);
    ...
    output[i++] = '$';
    std::snprintf(output + i, 4, "%2.2u$", logr);
    encode_base64(reinterpret_cast<std::uint8_t*>(output) + i + 3, csalt,
      BCRYPT_MAXSALT);
    encode_base64(reinterpret_cast<std::uint8_t*>(output) + std::strlen(output),
  ...
  #undef CHAR64
  #undef BCRYPT_VERSION
  #undef BCRYPT_MAXSALT
  #undef BCRYPT_BLOCKS
  #undef BCRYPT_MINROUNDS
  #undef BCRYPT_SALT_OUTPUT_SIZE
  #undef BCRYPT_OUTPUT_SIZE
  #undef _PASSWORD_LEN
  #undef BLF_N
  #undef BLF_MAXKEYLEN
  #undef F
