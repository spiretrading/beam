...
void Beam::Python::ExportWebServices(pybind11::module& module) {
  ...
  ExportUri(submodule);
  ExportWebSocketConfig(submodule);
  ExportWebSocket(submodule);
...
void Beam::Python::ExportWebSocketConfig(pybind11::module& module) {
  class_<WebSocketConfig>(module, "WebSocketConfig").
    def(init()).
    def(init<const WebSocketConfig&>()).
    def("set_uri", &WebSocketConfig::SetUri).
    def("set_version", &WebSocketConfig::SetVersion).
    def("set_protocols", &WebSocketConfig::SetProtocols).
    def("set_extensions", &WebSocketConfig::SetExtensions);
}

void Beam::Python::ExportWebSocket(pybind11::module& module) {
  using WebSocket = WebServices::WebSocket<std::unique_ptr<ChannelBox>>;
  class_<WebSocket>(module, "WebSocket").
    def(init(
      [] (const WebSocketConfig& config) {
        return std::make_unique<WebSocket>(config, TcpSocketChannelFactory());
      }), call_guard<GilRelease>()).
    def_property_readonly("uri", &WebSocket::GetUri).
