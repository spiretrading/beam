...
void CachedDataStoreTester::TestBackwardCoherence() {
  ...
  baseDataStore.Store(entryB);
  {
    BasicQuery<string> query;
    query.SetIndex("hello");
    query.SetRange(Sequence(108), Sequence(109));
  ...
  }
  {
    ...
    auto queryResult = dataStore.Load(query);
    CPPUNIT_ASSERT(queryResult.size() == 2);
    CPPUNIT_ASSERT(queryResult[0].GetSequence() == Sequence(106));
    CPPUNIT_ASSERT(queryResult[1].GetSequence() == Sequence(107));
  }
  {
    BasicQuery<string> query;
    query.SetIndex("hello");
    query.SetRange(Sequence(95), Sequence(105));
    query.SetSnapshotLimit(SnapshotLimit::Type::TAIL, 7);
    auto queryResult = dataStore.Load(query);
    CPPUNIT_ASSERT(queryResult.size() == 7);
    CPPUNIT_ASSERT(queryResult.front().GetSequence().GetOrdinal() == 99);
    CPPUNIT_ASSERT(queryResult.back().GetSequence().GetOrdinal() == 105);
  }
}
