...
TEST_SUITE("QueryClientPublisher") {
  ...
  TEST_CASE("recover_reinitializes_disconnected_publishers") {
    auto fixture = Fixture();
    auto query_count = 0;
    auto& client = *fixture.m_server_client;
    QueryService::add_slot(out(client.get_slots()),
      [&] (auto& protocol_client, const TestQuery& received_query) {
        ...
        if(query_count == 1) {
          result.m_id = 20;
          result.m_snapshot.push_back(SequencedValue(
            TestEntry(1100, time_from_string("2024-01-15 10:30:00")),
            Beam::Sequence(1)));
        } else {
          REQUIRE(
            received_query.get_range().get_start() == Beam::Sequence(2));
          result.m_id = 20;
          result.m_snapshot.push_back(SequencedValue(
            TestEntry(1300, time_from_string("2024-01-15 10:30:02")),
            Beam::Sequence(3)));
        }
        return result;
      });
    ...
    fixture.m_publisher.submit(query, queue);
    auto snapshot_value = queue->pop();
    REQUIRE(snapshot_value.get_value().m_value == 1100);
    fixture.m_publisher.publish(SequencedValue(IndexedValue(
      TestEntry(1200, time_from_string("2024-01-15 10:30:01")),
      std::string("IndexG")), Beam::Sequence(2)));
    auto published_value = queue->pop();
    REQUIRE(published_value.get_value().m_value == 1200);
    fixture.m_accept_routine = spawn([&] {
      fixture.m_server_client.reset();
      fixture.m_server_client.emplace(fixture.m_server.accept(), init());
      register_query_types(
        out(fixture.m_server_client->get_slots().get_registry()));
      register_test_query_services(out(fixture.m_server_client->get_slots()));
      register_test_query_messages(out(fixture.m_server_client->get_slots()));
      QueryService::add_slot(out(fixture.m_server_client->get_slots()),
        [&] (auto& protocol_client, const TestQuery& received_query) {
          ++query_count;
          auto result = QueryResult<SequencedTestEntry>();
          REQUIRE(received_query.get_range().get_start() == Beam::Sequence(3));
          result.m_id = 20;
          result.m_snapshot.push_back(SequencedValue(
            TestEntry(1300, time_from_string("2024-01-15 10:30:02")),
            Beam::Sequence(3)));
      ...
      fixture.m_server_client->spawn_message_handler();
    });
    fixture.m_accept_routine.wait();
    fixture.m_publisher.recover(*fixture.m_client_handler.get_client());
    auto recovered_value = queue->pop();
    REQUIRE(recovered_value.get_value().m_value == 1300);
    REQUIRE(query_count == 2);
  }
