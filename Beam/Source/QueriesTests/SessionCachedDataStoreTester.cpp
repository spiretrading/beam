...
namespace {
  ...
  using DataStore =
    SessionCachedDataStore<BaseDataStore*, EvaluatorTranslator<QueryTypes>>;
}
...
TEST_SUITE("SessionCachedDataStore") {
  TEST_CASE("store_and_load") {
    ...
    auto entry_a = store(data_store, "hello", 100,
      time_from_string("2016-07-30 04:12:55:10"), sequence);
    sequence = increment(sequence);
    auto entry_b = store(data_store, "hello", 200,
      time_from_string("2016-07-30 04:12:55:11"), sequence);
    sequence = increment(sequence);
  ...
  TEST_CASE("forward_coherence") {
    ...
    auto entry_a = SequencedValue(IndexedValue(
      TestEntry(100, time_from_string("2016-07-30 04:12:55:10")),
      std::string("hello")), sequence);
    base_data_store.store(entry_a);
    sequence = increment(sequence);
    auto entry_b = SequencedValue(IndexedValue(
      TestEntry(200, time_from_string("2016-07-30 04:12:55:11")),
      std::string("hello")), sequence);
    ...
    auto timestamp = time_from_string("2016-07-30 04:12:55:13");
    for(auto i = Beam::Sequence(102); i < Beam::Sequence(130);
        i = increment(i)) {
      auto ordinal = static_cast<int>(i.get_ordinal());
      auto entry = SequencedValue(IndexedValue(TestEntry(
        ordinal, timestamp + seconds(ordinal - 102)), std::string("hello")), i);
