...
void FunctionReactorTester::TestOneParameterWithSingleEval() {
  ...
  CPPUNIT_ASSERT(sequenceNumbers->Top() == 1);
  sequenceNumbers->Pop();
  AssertValue(*reactor, 1, BaseReactor::Update::EVAL, Square(911), false);
  p1->SetComplete();
  CPPUNIT_ASSERT(sequenceNumbers->Top() == 2);
  sequenceNumbers->Pop();
  AssertValue(*reactor, 2, BaseReactor::Update::COMPLETE, Square(911), true);
  AssertValue(*reactor, 3, BaseReactor::Update::NONE, Square(911), true);
}
...
void FunctionReactorTester::TestOneParameterWithMultipleEvals() {
  ...
  CPPUNIT_ASSERT(sequenceNumbers->Top() == 2);
  sequenceNumbers->Pop();
  AssertValue(*reactor, 2, BaseReactor::Update::EVAL, Square(416), false);
  p1->SetComplete(DummyException{});
  CPPUNIT_ASSERT(sequenceNumbers->Top() == 3);
  sequenceNumbers->Pop();
  AssertException<DummyException>(*reactor, 3, BaseReactor::Update::EVAL, true);
  AssertException<DummyException>(*reactor, 4, BaseReactor::Update::NONE, true);
}
...
void FunctionReactorTester::TestOneParameterWithFilter() {
  ...
  CPPUNIT_ASSERT(sequenceNumbers->Top() == 2);
  sequenceNumbers->Pop();
  AssertValue(*reactor, 2, BaseReactor::Update::EVAL, 10, false);
  p1->SetComplete();
  CPPUNIT_ASSERT(sequenceNumbers->Top() == 3);
  sequenceNumbers->Pop();
  AssertValue(*reactor, 3, BaseReactor::Update::COMPLETE, 10, true);
  AssertValue(*reactor, 4, BaseReactor::Update::NONE, 10, true);
}
